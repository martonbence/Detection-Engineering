...
| --------------- | ------------------------------------------------------ | ----------------------- | --------------- | -------- | --------------- |
| [DETECT-2026-0019](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0019_LSASS-Memory-Access-by-Non-Standard-Process.yml) | LSASS Memory Access by Non-Standard Process           | .001                    | Sysmon EID 10   | critical | Az LSASS folyamat memóriájában titkosítatlanul állnak a bejelentkezett felhasználók hash-ei/jegyei; egy nem-AV/EDR folyamat, ami ide nyit handle-t, tipikusan credential dumping kísérlet. |
| [DETECT-2026-0020](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0020_LSASS-Dump-via-comsvcs-MiniDump.yml) | LSASS Credential Dump via comsvcs.dll MiniDump        | .001                    | Sysmon EID 1    | critical | A comsvcs.dll (aláírt Windows-rendszerfájl) MiniDump exportját LOLBIN-ként visszaélve dumpolják le vele az LSASS-t, hogy külső eszköz (Mimikatz stb.) nélkül, kevésbé gyanúsan jussanak a memóriaképhez. |
| [DETECT-2026-0021](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0021_LSASS-Dump-via-ProcDump.yml) | LSASS Dump via ProcDump                                | .001                    | Sysmon EID 1    | critical | A Sysinternals ProcDump legitim, Microsoft-aláírt diagnosztikai eszköz — pont ezért kedvelt LSASS-dumpoláshoz: kevésbé tűnik gyanúsnak, mint egy dedikált credential-dumping tool. |
| [DETECT-2026-0025](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0025_LSASS-Dump-via-TaskManager-and-FileCreate.yml) | LSASS Memory Dump via Task Manager or Suspicious File Creation | .001            | Sysmon EID 11   | critical | A Task Manager beépített "Create dump file" funkciója GUI-ból, külön eszköz nélkül is képes LSASS-t dumpolni — alacsony belépési küszöbű módszer, amit csak a keletkező dump-fájl jelez. |
| [DETECT-2026-0023](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0023_SAM-and-LSA-Secrets-Dump-via-Registry-Hive-Export.yml) | SAM and LSA Secrets Dump via Registry Hive Export     | .002, .004              | Sysmon EID 1    | critical | A SAM/LSA registry hive-ok offline tartalmazzák a helyi jelszó-hash-eket és titkosított LSA Secrets-eket; `reg.exe save`-fel élő rendszer megállítása nélkül fájlba exportálhatók, majd offline törhetők. |
| [DETECT-2026-0026](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0026_SAM-and-LSA-Hive-Copy-via-esentutl.yml) | SAM and LSA Credential Hive Copy via esentutl         | .002, .004              | Sysmon EID 1    | critical | Az esentutl.exe (LOLBIN) VSS-integrált másolási funkciójával a futás közben zárolt SAM/SYSTEM hive fájlok közvetlenül másolhatók, explicit shadow copy létrehozása nélkül. |
| [DETECT-2026-0027](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0027_Credential-Hive-Access-via-Volume-Shadow-Copy.yml) | Credential Hive Access via Volume Shadow Copy         | .002                    | Sysmon EID 1    | critical | A Volume Shadow Copy szolgáltatás egy pillanatképet készít a lemezről, amin keresztül a futás közben zárolt SAM/SYSTEM/NTDS fájlok a fájlzárolás megkerülésével olvashatók ki. |
| [DETECT-2026-0024](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0024_NTDS-Credential-Dump-Attempt.yml) | NTDS.dit Credential Dump Attempt                       | .003                    | Sysmon EID 1    | critical | Az NTDS.dit a teljes domain összes felhasználójának hash-ét tartalmazza egy DC-n — a legmagasabb értékű célpont, mert egyetlen fájl kiolvasásával a teljes domain kompromittálható. |
| [DETECT-2026-0030](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0030_Symlink-Creation-to-Volume-Shadow-Copy-Device-Path.yml) | Symlink Creation to Volume Shadow Copy Device Path    | .003                    | Sysmon EID 1    | critical | Egy szimbolikus link a shadow copy eszközútvonalára (`\\?\GLOBALROOT\Device\HarddiskVolumeShadowCopy...`) egy sima fájlműveletnek álcázva teszi elérhetővé a mögöttes NTDS/SAM fájlt, külső VSS-eszköz nélkül. |
| [DETECT-2026-0031](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0031_Credential-Hive-Access-via-Low-Level-NTFS-Acquisition.yml) | Credential Hive Access via Low-Level NTFS Acquisition | .003                    | Sysmon EID 1    | critical | Nyers, kötet-szintű NTFS-olvasással (pl. RawCopy) a fájlrendszer feletti OS-zárolás teljesen megkerülhető, mert a művelet sosem a normál fájlkezelő rétegen keresztül fut. |
| [DETECT-2026-0028](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0028_Cached-Credential-Enumeration-via-Cmdkey.yml) | Cached Credential Enumeration via Cmdkey              | .005                    | Sysmon EID 1    | low      | A beépített `cmdkey /list` paranccsal a gépen tárolt cache-elt hitelesítő adatok felsorolhatók — alacsony zajszintű felderítési lépés, ami gyakran egy tényleges dump-kísérletet előz meg. |
| [DETECT-2026-0029](https://github.com/martonbence/Detection-Engineering/blob/main/rules/sigma/DETECT-2026-0029_DCSync-via-DSInternals-Get-ADReplAccount.yml) | DCSync via DSInternals Get-ADReplAccount              | .006                    | Sysmon EID 1    | high     | A DCSync egy DC-nek adja ki magát a domain-replikációs protokollon (MS-DRSR) keresztül, és lekéri a jelszó-hash-eket más DC-któl — nem fájl-hozzáférés, hanem jogosultsággal (Replicating Directory Changes) visszaélő hálózati technika. |
