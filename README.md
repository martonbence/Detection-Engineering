...
*Generated at 2026-07-25T09:40:44 UTC*
<!-- STATS_END -->
...
| `open_promotion_pr` | `ubuntu-latest` | Runs only `if: needs.splunk_verify.result == 'success'`. No checkout — pure `gh` CLI. Opens the promotion PR described below and sets its Project #3 status to `In review`. |
...
Opening the promotion PR is its own job, `open_promotion_pr`, not a step inside `splunk_verify`. It runs on plain `ubuntu-latest` (no lab access needed — it's pure `gh` CLI, no checkout), gated with `needs: splunk_verify` and `if: needs.splunk_verify.result == 'success'`. When that condition holds, it opens a pull request (unless one is already open) with `gh pr create --base main --head dev --title "Promote verified detections from dev to main" --label automated-promotion`. This PR does **not** auto-merge — a human reviews and merges it manually, which is what triggers `ci_prod_workflow.yml` to deploy to prod. The same job immediately adds the new PR to [Project #3](https://github.com/users/martonbence/projects/3) (`gh project item-add`) and sets its Status field to `In review` (option `4fdb6324`), so auto-opened promotion PRs land on the board pre-triaged instead of mixed in with manually-created Todo/Ready items.

Gating on `needs.splunk_verify.result` (rather than reading a job-level `outputs.exit_code` from `splunk_verify`) is a deliberate fix, not an oversight: `splunk_verify`'s own `if:` starts with `always()` so it still runs when an atomic-test job upstream failed or was skipped, and cross-job outputs from an `always()`-gated job were observed not to reliably propagate into a downstream job's `if:` context — a run with a genuine 5/5 PASS and "Final verdict: PASS" in `splunk_verify`'s own log still had `open_promotion_pr` skip with zero steps recorded when it depended on that output. `splunk_verify`'s last step, `Report verification verdict`, re-exits with the real PASS/FAIL code, so the job's own `result` already *is* the verdict — no output needed.
...
| Runner label(s) | Role |
|---|---|
| `ubuntu-latest` | Validate/convert/bundle steps, `open_promotion_pr` (pure `gh` CLI, no checkout), and both GitHub Pages publish jobs — no access to lab infrastructure needed. |
