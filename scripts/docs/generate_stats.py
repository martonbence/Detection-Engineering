...
def render_html_summary(stats: dict, repo: str) -> str:
    ...
    def verdict_html(verdict: str, run_id: str) -> str:
        ...
        badge = f'<span class="badge" style="background:{bg};color:#fff">{_html.escape(verdict)}</span>'
        if run_id:
            url = f"https://github.com/{repo}/actions/runs/{run_id}"
            return f'<a href="{url}" target="_blank" title="View Actions run &amp; download artifact">{badge}</a>'
        return badge
    ...
    for r in stats["rules"]:
        ...
        tac_plain = _html.escape(", ".join(tactics) or "—")

        techniques = r.get("techniques") or []
        tech_parts = [f'<a href="{technique_url(t)}" target="_blank">{_html.escape(t)}</a>' for t in techniques]
        tech_cell = "<br>".join(tech_parts) or "—"
        tech_plain = _html.escape(", ".join(techniques) or "—")

        sev_order = SEV_ORDER.index(lvl) if lvl in SEV_ORDER else 99
        ...
        rows.append(
            ...
            f'<td>{source}</td>'
            f'<td data-search="{tac_plain}">{tac_cell}</td>'
            f'<td data-search="{tech_plain}">{tech_cell}</td>'
            f'<td data-search="{_html.escape(lvl.capitalize())}" data-order="{sev_order}">{sev_badge(lvl)}</td>'
            f'<td>{status}</td>'
...
<head>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    table.dataTable tbody td.title-cell {{ text-align: left; white-space: nowrap; }}
...
<body>
  ...
  <div class="stats-row">
    ...
    <div class="stat-card">
      <div class="stat-value" style="color:#2EA44F">{passed}</div>
      <div class="stat-label">Pass</div>
    </div>
    <div class="stat-card">
      <div class="stat-value" style="color:#CF222E">{failed}</div>
      <div class="stat-label">Fail</div>
    </div>
    <div class="stat-card">
      <div class="stat-value">{not_ver}</div>
      <div class="stat-label">Not Verified</div>
    </div>
    <div class="stat-card">
      <div class="stat-value" style="color:#2EA44F">{pass_rate}%</div>
...
{rows_html}
  ...
  <script>
    $(function() {{
      ...
      // Add select dropdown filters to Source (2), Severity (5), Status (6), Verdict (7)
      [2, 5, 6, 7].forEach(function(colIdx) {{
        var col = table.column(colIdx);
        var th = $(col.header());

        var uniqueVals = [];
        $('#rules-table tbody tr td:nth-child(' + (colIdx + 1) + ')').each(function() {{
          var val = $(this).attr('data-search') || $(this).text().trim();
          if (val && val !== '—' && uniqueVals.indexOf(val) === -1) uniqueVals.push(val);
        }});
        uniqueVals.sort();
