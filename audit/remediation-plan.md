...
Jelenlegi: **6,5 / 10** (kiindulás). Mai állás: **8,59 → 8,6 / 10**, kész súly **77,5 / 92,5**
(45 tétel az 54-ből) — 2026-08-11-én a **2.22** (generátor-regex a séma technika-mintájához
igazítva) és a **4.9** (érdemi promotion PR body, per-szabály tábla) zárult le. 2026-08-10-én a
...
- [ ] **3.8** Flat rule-könyvtár és manuális ID-kiosztás (lyukak: 0001, 0002, 0004, 0017) · fix: alkönyvtárak + 4.5 scaffolder · **2026-08-11: az alkönyvtár-fele megbeszélve és a felhasználó által elutasítva.** Indoklás: a rule-böngésző a YAML *tartalma* alapján kategorizál (tactic/technika/log-source szűrők), nem a könyvtárszerkezet alapján, a fájlnevek beszédesek, a fájlrendszer/grep/fuzzy-find több száz fájllal is elbír egy könyvtárban — az eredeti "27 még kezelhető, 150 már nem" állítás nem volt tényleg alátámasztva. Tactic-alapú bontást külön is elvetettünk volna, mert egy szabály jellemzően több `attack.*` taget visel, egy elsődleges tactic-mappa mesterséges lenne. **A "manuális ID-kiosztás" fele viszont valós, konkrét kockázat** (két párhuzamos branch ugyanazt a szabad ID-t választhatja, semmi nem veszi észre) — ez a **4.5** alatt kezelve, alkönyvtárak nélkül
...
- [ ] **4.5** `new_rule.py` scaffolder (következő szabad `detect_id`, séma-konform skeleton) + `Makefile` a lokális futtatáshoz · **a scaffolder + ütközésvédelem fele kész 2026-08-11, a Makefile és az ATT&CK→atomic-teszt-lookup szándékosan kimaradt, ezért nincs pipa.** Új `scripts/new_rule.py`: a következő szabad `DETECT-<év>-NNNN`-t a `lib.rules.discover()`/`load_rule()`/`detect_id()`-re építve számolja (soha nem tölt ki lyukat — egy visszavont ID újrahasznosítása régi verdikt-history-t/coverage-pontokat keverne össze), a szerzőt `git config user.name`-ből olvassa, és egy séma-konform TODO-skeletont ír ki. Új `scripts/validate/check_detect_id_uniqueness.py`, bekötve a `ci_dev_workflow.yml`-be (`check_test_routing.py` és `check_mitre_tags.py` közé) — ez a tényleges védőháló, mert a scaffolder csak a helyi checkoutot látja, két párhuzamos branch ugyanarról a base commitról ugyanazt a "következő szabad" ID-t számolhatja ki. Lásd a Napló-bejegyzést
