...
Jelenlegi: **6,5 / 10** (kiindulás). Mai állás: **9,0 / 10**, kész súly **92,5 / 92,5**
(**54 tétel az 54-ből — a register teljes**) — 2026-08-15-én a **4.6** zárult le, élesben
bizonyítva egy valódi `LAB_ONLINE=true` futással: 26 szabály valódi `history.jsonl`-t kapott, a
dashboard sparkline élesen renderelődött (lásd a Napló végét). Korábban: (53 tétel az 54-ből) —
a **4.8** is elutasítással zárult (a meglévő rule browser drawer már lefedi a tétel nagy részét,
a maradék nem indokolt jelenleg). Még korábban: (52 tétel az 54-ből) — a **4.1** is elutasítással
zárult (nincs háttérforgalom a
labor Splunkban, csak attack-teszt idején van adat — egy "csendes ablak" mérés értelmezhetetlen
lenne). Még korábban: (51 tétel az 54-ből) — a **4.4** elutasítással zárult (nincs
...
- [x] **4.6** Per-rule verdict history (append-only `history.jsonl`) a „flaky / mikor romlott el" kérdésekhez · **kész 2026-08-15, élesben bizonyítva.** Az infrastruktúra (`scripts/lib/verdict_history.py`, a `pass_fail_eval.py` append-only írása, a workflow append-merge logikája, a `generate_stats.py::_deployment_sparkline()` szerver-oldali SVG-renderelés) már 2026-08-12-én megépült (`9671385`), de sosem futott éles CI-n — a `LAB_ONLINE` hetek óta `false` volt. Miután a felhasználó `LAB_ONLINE=true`-ra állította és egy valódi 26-szabályos `workflow_dispatch`-et futtatott (run `31896815997`), a `Deploy to Splunk` job közben egy addig ismeretlen, valódi hiba derült ki és lett javítva (lásd a **4.2** Napló-utókövetését — a `check_spl_syntax.py` `search` prefix nélkül küldte a query-t a `search/v2/parser`-nek, ami minden szabályt elbuktatott; ez az első alkalom volt, hogy ez a script élő Splunk ellen futott). A javítás után a megismételt futás sikeresen deployolt, támadott (Atomic Red Team + Script Emulation, mind zöld) és verifikált: **25/26 PASS, 1 FAIL** (`DETECT-2026-0019`, 11 esemény a 10-es max fölött — ez a **2.7** tételben már megjósolt, ismert, nem-hiba flakiness, nem pipeline-bug). A `Commit verification results` lépés valódi `outputs/results/<detect_id>/history.jsonl`-t pusholt mind a 26 szabályhoz (`8fcd2a9..5c67d6d`), egy JSON-sort/futás (`run_timestamp`, `verdict`, `event_count`, `rule_version`, `git_sha`, `run_id`). **Ellenőrizve, nem feltételezve:** a `docs/index.html`-ben a `DETECT-2026-0019` sora egy piros `dep-spark-fail` SVG-pöttyöt kapott helyes tooltippel (`2026-08-15 17:03:45 · FAIL · 1.6`), a `0020`/`0021` zöld `dep-spark-pass` pöttyöt — a sparkline ténylegesen, élesben, a valódi history-ból renderelődött, nem csak statikusan ellenőrizve. **Innentől a register teljes: 54/54.**
...
- [x] **4.8** A `rule_documentations/` generálása a `stats.json`-ból — megvizsgálva, elutasítva: nem indokolt · **lezárva 2026-08-15, elutasításként, nem megvalósításként.** Mielőtt bármit terveztem volna, megnéztem a mai állapotot: a rule browsernek már ma is van egy per-szabály drawerje (`scripts/docs/assets/page.js:1789-1889`), ami leírást, metaadatot, MITRE ATT&CK-et, referenciákat, verifikáció-státuszt és a teljes szabálydefiníciót mutatja — a tétel eredeti céljának nagy része már megvan, csak nem "runbook" vagy `rule_documentations/` formában. Ami valóban hiányzik a drawerből (`falsepositives`, `testing` config, teljes `logsource`, verdikt-history a **4.6** alá tartozik) egy kisebb, célzott bővítés lenne, nem egy új generátor-alrendszer. A korábbi `rule_documentations/` könyvtár (törölve 2026 áprilisában, sosem volt valódi tartalma, csak `test.txt` placeholder) egy korai, azóta elhagyott elképzelés maradványa. A felhasználó mindkét irányt (drawer-bővítés, illetve egy tényleg különálló, offline olvasható per-szabály oldal-generátor) átgondolta, és egyiket sem tartja jelenleg indokoltnak — a drawer a gyakorlatban elég. Ugyanaz a minta negyedszer: a **3.8**, **4.4**, **4.1** után a **4.8** is egy olyan sablon-tétel volt, ami nem ennek a konkrét repónak a mai állapotára lett szabva. Kód nem változott, csak a register
...
- **2026-08-15 — 4.8 lezárva elutasításként, mielőtt bármit implementáltunk volna.** A felhasználó
  megkérdezte, miről szól a tétel — mielőtt válaszoltam volna, megnéztem a mai `page.js` drawer
  kódját (nem csak a tétel szövegét), és kiderült: a "runbook-oldal szabályonként" cél nagy része
  már megvalósult, csak nem `rule_documentations/` formában — a rule browser drawerje leírást,
  metaadatot, MITRE-et, referenciákat, verifikáció-státuszt és a teljes szabálydefiníciót mutatja.
  Ezt részleteztem, plusz azt is, hogy mi hiányzik ténylegesen (`falsepositives`, `testing`,
  teljes `logsource`) és két utat javasoltam: kis drawer-bővítés, vagy egy tényleg különálló,
  offline olvasható per-szabály generátor (mint a 2026 áprilisában törölt, sosem élt
  `rule_documentations/`). **A felhasználó egyiket sem tartja jelenleg indokoltnak** — elutasítva.
  Negyedik eset ugyanabból a mintából: a **3.8**, **4.4**, **4.1** után a **4.8** is egy sablon-
  tétel volt, ami erre a konkrét repóra és annak mai állapotára nem lett külön szabva. Kód nem
  változott, csak a register.
  **Innentől a register minden tétele eldőlt: 53 kész, 1 nyitva (4.6, a tervezett teljes
...
- **2026-08-15 — 4.6 lezárva, a register utolsó nyitott tétele.** A felhasználó `LAB_ONLINE=true`-ra
  állította a repo variable-t és egy 26-szabályos, PASS/N/A-szűrt `workflow_dispatch`-et indított
  (`31896204938`), hogy egy valódi deploy+attack+verify ciklussal zárja le a 4.6-ot. Az első futás
  a `Deploy to Splunk` jobnál elbukott — mind a 19 vizsgált szabály `Unknown search command 'index'.`
  hibát kapott a `check_spl_syntax.py`-tól (4.2), ami a nyers `index=...` query-t `search` prefix
  nélkül küldte a `search/v2/parser`-nek; a `search` maga generating command, amit a `saved/searches`
  endpoint (a tényleges deploy) automatikusan hozzáfűz, a parser endpoint viszont nem — ezt a hibát
  semmilyen mock nem tudta volna elkapni, mert ez volt az első alkalom, hogy ez a script élő
  Splunk ellen futott (a `LAB_ONLINE` hetek óta `false` volt). A javítás (lásd a 4.2 Napló-bejegyzését
  fent) után a felhasználó újra pusholta a fixet és megismételte a dispatchot (`31896815997`).
  **Ez a második futás sikeres volt:** `Deploy to Splunk`, `Script Emulation Test`, mindkét
  `Atomic Red Team Test` zölden lefutott — ez volt az első alkalom, hogy ez a 26 szabály ténylegesen
  élesben deployolt és támadva lett a mai pipeline-lal. A `Splunk Verification` job 25/26 PASS-t
  adott, egy FAIL-lel (`DETECT-2026-0019`, 11 esemény > 10-es max) — ez a **2.7** tételben már
  előre jelzett, ismert flakiness (a szabály korábban pontosan 10 eseményen állt), nem a mostani
  munka hibája. A job végső lépése (`Report verification verdict`) szándékosan pirosra fut, ha
  bármelyik szabály FAIL — ez a design, nem hiba —, de a `Commit verification results` lépés előtte
  sikeresen lefutott és pusholt.
  **Ellenőrizve élesben, nem feltételezve:** `git ls-tree` a pusholt `outputs/results/`-ban 26 új
  `history.jsonl`-t mutat, tartalmuk valódi mért adat (`DETECT-2026-0019`: `{"run_timestamp":
  "2026-08-15T17:03:45...", "verdict": "FAIL", "event_count": 11, "rule_version": "1.6",
  "git_sha": "7d82cf1...", "run_id": "31896815997"}`). A pusholt `docs/index.html`-ben a
  `_deployment_sparkline()` a valódi historyból helyesen renderelt: `DETECT-2026-0019` piros
  `dep-spark-fail` pöttyöt kapott a helyes tooltippel, `0020`/`0021` zöldet (`dep-spark-pass`).
  **A register ezzel teljes: 54/54 tétel eldőlt (49 megvalósítva, 4 tudatosan elutasítva —
  3.8, 4.1, 4.4, 4.8, mindegyik a repo tényleges állapotának/korlátainak ELŐZETES megnézése után,
  nem vakon), pontszám 9,0/10 — a formula szerinti maximum.**

- **2026-08-29 — 3.5 utókövetés: a 2026-08-11-es bejegyzésben tudatosan nyitva hagyott séma-kérdés
  is lezárult, a pipa és a pontszám érintetlen.** A 3.5 saját szövege ezt írta a duplikáció-fél
  lezárásakor: „A séma maga — hogy a »verzió« git commit-számból származik, nem valaki tudatos
  döntéséből … szándékosan nyitva maradt, önálló, nagyobb döntésként (explicit `version:` mező a
  YAML-ben + CI-gate a bumpra, minden rule-fájlt érintve)." A 2026-08-15-i lezárás ennek csak a
  fegyelem-felét oldotta meg (kézzel karbantartott `version:` mező + `check_version_bump.py`
  CI-gate), és a saját szövege szerint `scripts/lib/rule_version.py`-t és a git-commit-számból mért
  `rule_version`-t **tudatosan érintetlenül hagyta**, mint a kézzel állított mezőtől „két külön
  dolog, egymás mellett."

  Ma ez a maradék fele is lezárult, a `cd40d34` commit-tal: `scripts/lib/rule_version.py` és
  `compute_rule_version()` **törölve** (ellenőrizve: a fájl a jelenlegi fastruktúrában nem létezik,
  `git log --stat cd40d34` a törlést mutatja, 85 sor `-`). A kézzel állított `version:` mező lett
  az **egyetlen** forrás mindenhol — `sigma_to_spl.py::build_meta_dict()` és `generate_stats.py`
  most közvetlenül ezt olvassa git-hívás helyett (ellenőrizve a forráskódban, ld. a `# Register
  item 3.5, closed for real` kommenteket mindkét fájlban). Új `.githooks/pre-commit` (ellenőrizve,
  316 sor): a staged Sigma-szabályokon a `check_version_bump.py`-ból **importált** (nem
  újraimplementált) `logic_diff()`/`version_of()`-fal nézi, változott-e `detection:`/`logsource:`/
  `custom.splunk.raw_query` a HEAD-hez képest, és ha igen és a `version:` még nem mozdult kézzel,
  MINOR-t bump-ol és `git add`-olja vissza — sosem bukik, sosem blokkol (minden kivétel elnyelve,
  0-s exit), a `check_version_bump.py` CI-gate marad a backstop a `--no-verify`, friss klón (a hook
  csak `git config core.hooksPath .githooks` egyszeri futtatása után aktív — ez ebben a repóban
  megtörtént, ellenőrizve: `git config core.hooksPath` → `.githooks`) vagy GitHub web UI-s szerkesztés
  eseteire. Egyszeri `scripts/migrate_backfill_rule_version.py` (ellenőrizve, létezik és a docstringje
  szerint valódi történelem-replay, nem a régi commit-számláló újrafuttatása) minden meglévő szabály
  `version:`-jét a `logic_diff()` teljes git-history-n való visszajátszásával számolta újra — 27/28
  szabály változott.

  **Ugyanaznap talált és javított kapcsolódó hiba, `4bc4ea5`:** a `scripts/state/select_unverified.py`
  saját `git_version()`-je a migráció után is a régi commit-számláló sémát számolta függetlenül,
  miközben minden `result.json` már az új, YAML-eredetű `rule_version`-t hordozza — a két
  össze-nem-tartozó séma összehasonlítása miatt egy manuális `workflow_dispatch` unverified futás
  szinte minden szabályt kiválasztott volna a valós elavultságtól függetlenül. Ellenőrizve: a script
  most `data.get("version")`-t olvas közvetlenül (`generate_stats.py` mintáját követve),
  `git_version()` törölve. Mindkét commit-ot Bjorn nézte át (nem blokkoló észrevétel: teszt-lefedettség
  hiánya az új hook/migrációs script körül — nincs `tests/test_pre_commit*`, sem
  `tests/test_migrate_backfill*`, ellenőrizve, hogy a repóban valóban nincs ilyen fájl), Gaz
  commitolta. `register.html` (2026-08-15-i generálás óta nem frissült) és `remediation-plan.md`
  maga (a `facae74` óta ez az első módosítása) egyaránt utólag frissül ezzel a bejegyzéssel — a 3.5
  pipája és a register pontszáma (9,0/10) nem mozdul, ez bookkeeping, nem újranyitás.
