...
- **2026-08-24 (későbbi, önálló kör, ugyanazon a napon mint a fenti
  2. szelet napló) — 4.1 negyedik napló, 3. szelet lezárva (3. az
  5-ből), Kwame verifikálta.** `git log --oneline` → `8566f90`
  („refactor(ci): extract promotion-PR step to testable Python (4.1,
  slice 3)"), rögtön `10d30e3` után („chore(audit): book 4.1 slice 2,
  Kwame-verified"), ma, 2026-08-24 18:01-kor. `git diff --numstat
  8566f90^ 8566f90` egyezik a jelentéssel pontosan: `ci_dev_workflow.yml`
  **+30/−132** (nettó −102), `scripts/state/open_promotion_pr.py` új
  fájl **+525**, `tests/test_open_promotion_pr.py` új fájl **+449** —
  összesen +1004/−132, ahogy a commit `--stat`-ja is mutatja.
  **Viselkedés-egyezés a régi shell ellen, sorról sorra ellenőrizve:**
  `git show 8566f90^:.github/workflows/ci_dev_workflow.yml` (`fe755d5`
  is ugyanaz) vs. az új `open_promotion_pr.py` diffelve — a két nem
  best-effort `gh` hívás (`gh pr list`, `gh pr create`, mindkettő
  `check=True`, kivétel a `subprocess.CalledProcessError`-en keresztül
  jut vissza `main()`-be és ugyanazzal a kilépési kóddal áll meg, mint a
  régi shell `set -euo pipefail` alatt) helyesen különbözik a másik
  négy best-effort hívástól (`fetch_stats_json`, `fetch_compare_filenames`,
  `add_to_project`, `set_project_status`, mind `check=False`, hibára
  csendben visszaesnek); a `stale_count = verified_expired +
  verified_superseded` számítás nullára esik vissza minden hibaágon
  (`compute_stale_count`); a `rule_table` építése (`DETECT-####-####`
  kinyerés, dedup+sort `extract_changed_rule_ids`-ben, egy `stats.json`-
  ból hiányzó id csendes eldobása `build_rule_table`-ben) ténylegesen
  dokumentált, tesztelt viselkedés
  (`test_rule_table_missing_id_silently_dropped_not_flagged`), nem
  véletlen eltérés; a PR body / step-summary összeállítás mind a négy
  stale_note/rule_table jelenlét-hiány kombinációra le van fedve
  (`test_pr_body_no_stale_note_no_rule_table` … `_both`); a két
  Project-board best-effort hibaág egymástól függetlenül tesztelt
  (`test_run_step_project_add_failure_warns_and_does_not_call_edit`,
  `test_run_step_project_edit_failure_warns_after_successful_add`).
  Egyetlen apró, viselkedést nem érintő sorrendi eltérés: a régi shell a
  `gh api compare` hívást akkor is megtette, ha a `stats_json` nem üres,
  de érvénytelen JSON volt (csak `-n "$stats_json"`-t nézett); az új
  Python ezt `stats is not None`-nal dönti el, tehát ilyen (valójában
  soha elő nem forduló, mert a `gh api ... | jq -r`-en keresztül jövő
  válasz vagy üres, vagy érvényes JSON) élen egy hívással kevesebbet
  tenne — a végeredmény (üres tábla) mindkét esetben azonos, ezt a
  `test_run_step_does_not_fetch_compare_when_stats_unavailable` teszt
  rögzíti is, csak nem ezzel a magyarázattal — nem hibás, nem is
  emeltem ki hibaként, csak a sorról sorra ellenőrzés miatt jegyzem meg.
  **A tudatos, megjelölt eltérés (escape_cell) valós és pontosan
  jellemzett:** `lib.summary.escape_cell()` ténylegesen létező,
  már a `de6e440`/`5f34f94` commitokban (jóval e szelet előtt)
  bevezetett, azóta öt másik scriptben (`pass_fail_eval.py`,
  `check_mitre_tags.py`, `check_spl_syntax.py`,
  `deploy_spl_to_splunk.py`, `check_version_bump.py`) is használt,
  bevett konvenció ebben a repóban — nem kitalált függvény. A `|`,
  newline escape-elése és trim. A mai 28 `rules/sigma/*.yml` cím
  (`title:`) egyike sem tartalmaz `|`-t, backtick-et vagy egyéb
  táblatörő karaktert (`grep -E '[|`]'` nulla találat), tehát a
  jelentés „egyetlen mai cím sem renderelődne másképp" állítása pontos
  a jelenlegi rule-készletre — a teszt (`STATS_WITH_RULES` fixture-je)
  külön egy `"Odd|Pipe Title"` címmel is fedi az escape-elést, tehát a
  viselkedés-változás maga is tesztelt, nem csak dokumentált.
  **A regiszter-idézés javítása valós és helyesen attribuált:** a
  workflow-kommentben (`ci_dev_workflow.yml:2462-2464` körül) és a
  script docstringjében egyaránt `audit/remediation-plan.md item 4.9`
  és `audit/remediation-plan.md item 4.11` szerepel, nem puszta
  „register item 4.9/4.11" — mindkét szám valóban létező,
  `remediation-plan.md`-beli, lezárt tétel (4.9: „A promotion PR
  body-ja legyen érdemi", kész 2026-08-11; 4.11: „Az elévülés
  riportálási korrekció, nem kapu", kész 2026-08-10), pontosan azzal a
  tartalommal, amit a kommentek leírnak — a `pipeline-ci-gotchas` skill
  I. szakaszának pontosan megfelelő javítás, nem téves attribúció.
  **A checkout/setup-python akciók SHA-pinningje valóban a megjelölt
  tagre mutat, GitHub API-n keresztül feloldva, nem a kommentből
  átvéve:** `actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1`
  → `refs/tags/v7.0.1` ugyanerre a SHA-ra mutat;
  `actions/setup-python@5fda3b95a4ea91299a34e894583c3862153e4b97` →
  `refs/tags/v7.0.0` ugyanerre a SHA-ra mutat — mindkettő pontosan
  egyezik a workflow-kommentben álló tag-jelöléssel.
  **A tesztelést és a kódot közvetlenül futtatva, nem csak átolvasva
  ellenőriztem:** friss venv-be telepítve a `.github/requirements-dev.txt`
  + `.github/requirements.txt` függőségeit, `pytest
  tests/test_open_promotion_pr.py -q` → **46 passed**; a teljes `pytest`
  suite → **767 passed**, 0 failed, kilépési kód 0 (767−46=721, egyezik
  a jelentett „up from 721" állítással, ami maga is egyezik a
  2026-08-24-i szelet-2 zárónapló saját mérésével); `ruff check .` a
  teljes repón → „All checks passed!"; a módosított `ci_dev_workflow.yml`
  `yaml.safe_load`-dal betöltve hiba nélkül fut. **`actionlint` v1.7.12
  + `shellcheck` v0.10.0 saját letöltésű binárisokkal (a jelentett
  verziószámokkal pontosan egyezően, `-version`/`--version`-nel
  ellenőrizve) futtatva mind a négy workflow-fájl ellen**
  (`ci_dev_workflow.yml`, `ci_prod_workflow.yml`, `ci_prod_audit.yml`,
  `ci_code_checks.yml`) → nulla találat, kilépési kód 0.
  **Regiszter-idézési konvenció (a `pipeline-ci-gotchas` skill I.
  szakasza) betartva, a Skill-eszközzel előbb betöltve.**
  **Miért csak a 3. szelet és nem lezárás:** a maradék két blokk, méret
  szerint csökkenő sorrendben (`splunk_verify` „Reconcile Splunk state
  against the repo" 118 sor, `prepare_validate_convert` „Build pipeline
  bundle" 111 sor) és a kisebb blokkok hosszú farka érintetlen. A
  nyitott halmaz **nem** mozdul — a 4.1 továbbra is részleges, a lezárt
  tételszám marad **37/52**. **Következő valós, nem-docs tétel:** a
  fenti két megnevezett blokk bármelyike (méret szerint: `splunk_verify`
  118, `prepare_validate_convert` „Build pipeline bundle" 111) vagy
  **4.4** maradék fele — Gaz dönt a sorrendről. **Modell:** ez a
  könyvelési kör Sonnet 5-ön futott.

- **2026-08-24 (harmadik, önálló könyvelési kör ma — a szelet-2 és a
  szelet-3 napló után — 4.1 ötödik napló, 4. szelet lezárva (4. az
  5-ből), Kwame verifikálta.** `git log --oneline` → `1a104c0`
  („refactor(ci): extract Splunk reconcile step to testable Python (4.1,
  slice 4)"), rögtön `87e01df` után („chore(audit): book 4.1 slice 3,
  Kwame-verified"), ma. `git diff --numstat 1a104c0^ 1a104c0` egyezik a
  jelentéssel pontosan: `ci_dev_workflow.yml` **+44/−128**,
  `scripts/state/reconcile_step.py` új fájl **+374**,
  `tests/test_reconcile_step.py` új fájl **+553** — összesen +971/−128,
  ahogy a commit `--stat`-ja is mutatja.
  **A `bash -e`/`if`-mentesség állítása valós és helyesen alkalmazott.**
  GitHub minden `shell: bash` lépést `bash --noprofile --norc -e
  -o pipefail`-ként futtat a lépés saját `set` sorától függetlenül, és
  egy `if` feltételen belüli parancs kivétel az `-e` alól — ez tényleg
  bash-viselkedés, nem a jelentés kitalációja, és a régi shell (`git show
  1a104c0^:.github/workflows/ci_dev_workflow.yml` 1831-1848. sor)
  ténylegesen ezért csomagolta `if python … | tee …; then rc=0; else
  rc="${PIPESTATUS[0]}"; fi`-be. A `reconcile_step.py::run_step()`-et
  átolvasva: nincs korai `return` és nincs elkapatlan kivétel-út, ami
  `rc != 0`-nál kihagyná az összegzés-írást vagy a figyelmeztetéseket —
  a sorrend (extra_args → subprocess → `write_summary()` FELTÉTEL
  NÉLKÜL → drift-blokk, ha van report → `rc != 0`-nál a második
  warning → `return rc`) pontosan a régi shell parancssorrendjét adja
  vissza, csak Python-ban ehhez nem kellett az `if`-trükk, mert egy
  függvény nemnulla visszatérése nem szakítja meg az interpretert —
  amit a script docstringje maga is pontosan így indokol.
  **A tee/PIPESTATUS-egyenérték `run_reconcile_subprocess()`-ben
  ellenőrizve, nem csak elfogadva:** `subprocess.Popen(cmd,
  stdout=subprocess.PIPE, stderr=None, text=True, bufsize=1)` — saját,
  önálló futtatással megerősítve, hogy Python `subprocess`
  szemantikájában `stderr=None` valóban a szülő stderr-jét örökli (nem
  kapja el), míg `stdout=PIPE` befogva marad: egy kis teszt-szkript
  `sys.stderr`-re írt sora közvetlenül a terminálra ment, a `stdout`-ra
  írt sor viszont a visszaadott listában landolt, kizárólag ott. Ez
  pontosan a régi `| tee` viselkedése (csak a stdout volt a pipe-ban), a
  visszaadott `proc.returncode` pedig a *reconcile.py* saját kilépési
  kódja, nem egy wrapperé — nincs `STDOUT`-ba olvasztás, nincs
  `stderr=PIPE` tévedés.
  **Az apply-előtti-állapot korrekció — a legterheltebb logika ebben a
  szeletben — mezőnév-szinten és aritmetika-szinten is egyezik.**
  `scripts/state/reconcile.py`-t átolvasva: a `report["counts"]` mezői
  (`missing`, `duplicate_names`), az `orphan_removed[].retired` és
  `orphan_renamed[].name`, valamint az `applied.actions[]` elemek
  (`{"action": "delete"|"retire", "name": …, "ok": …}`, ld.
  `apply_changes()` 415-465. sor) pontosan azok a mezőnevek és alakok,
  amiket a `reconcile_step.py::_applied_names()`,
  `corrected_unresolved_count()` és `corrected_renamed_count()`
  feltételez. A régi `jq` kifejezéseket (`git show
  1a104c0^:.github/workflows/ci_dev_workflow.yml` 1883-1894. sor) az új
  Pythonnal sorról sorra összevetve: az `unresolved` halmazkivonás
  (`orphan_removed` nem-retirált nevei mínusz az `applied.actions`
  sikeres `retire`-jei) és a `renamed` (az összes `orphan_renamed` név
  mínusz az `applied.actions` sikeres `delete`-jei) aritmetikailag
  azonos a `corrected_unresolved_count()`/`corrected_renamed_count()`
  halmazkivonásával — nem csak hasonló, hanem ugyanaz a két lépés
  (szűrés majd kivonás), csak `jq`-ból listakomprehenzióba írva. A `#42`/
  `#41` futásokra való hivatkozást nem kellett újra megtalálni, csak az
  aritmetikát ellenőriztem, ahogy a feladat is kérte.
  **A négy különálló remedy-mondat valóban négy külön mondat, nem sablon.**
  `build_drift_warning_parts()` a régi shell `parts+=(...)` négy `if`
  ágát (1909-1920. sor) adja vissza szó szerint, saját szöveggel
  mindegyik feltételre — a `missing`-ágé kifejezetten „these need a
  deploy, not a retirement", nem a `retire_orphans`-os mondat, tehát a
  jelentésben leírt régi hiba (egy hiányzó szabályról azt állítani, hogy
  „keeps running", és `retire_orphans`-t javasolni rá) ebben a Python
  verzióban nincs újra behozva.
  **A regiszter-idézés javítása valós és helyesen attribuált, a
  `pipeline-ci-gotchas` skill I. szakaszát a Skill-eszközzel előbb
  betöltve ellenőriztem.** `audit/remediation-plan.md`-ben mind a négy
  hivatkozott tétel létezik, és a workflow-kommentek pontosan azt írják
  le, amit a register maga rögzít: **1.7** (árva `.spl`/saved
  search/`deprecated`-kivezetés, kész 2026-08-04), **1.8** (átnevezés
  utáni árva saved search, a javasolt fix elutasítva, a tünetet a 3.3
  `orphan_renamed`-je kezeli), **2.18** (`SPLUNK_APP` secret→vars
  migráció, kész 2026-08-06) és **2.14** (a `SPLUNK_VERIFY_TLS` `||
  'false'` fallback hiánya, kész 2026-08-06). A két megjelölt ütközés is
  valós: az aktív register saját **2.14**-e egy teljesen más tétel
  („Nincs `CONTRIBUTING.md`", 117. sor), és a saját **4.7**-e szintén más
  („Nincs UI-regressziós ellenőrzés", 143. sor, Sienna+Jamal alá
  delegálva) — a workflow-kommentben szereplő disambiguáló zárójel
  („not this repo's active register's own 2.14, which is the unrelated
  CONTRIBUTING.md item") tehát pontos, nem csak plauzibilis.
  **A szándékosan érintetlenül hagyott ismert rés valóban ott van és
  valóban rés.** Az „Upload reconciliation report" lépés
  (`ci_dev_workflow.yml`, a reconcile lépés után közvetlenül, jelenleg
  1857. sor körül) `git diff 1a104c0^ 1a104c0`-ban nem szerepel — tehát
  változatlanul csupasz „register item 4.7"-et ír a kommentjében, ami a
  `remediation-plan.md` 4.7-ére utal (deployment inventory a
  dashboardon, kész 2026-08-08 — tartalmilag egyezik: „publishing it to
  the dashboard is register item 4.7"), de ütközik az aktív register
  saját, más tartalmú 4.7-ével. Ésszerű hatókör-döntés otthagyni: ez egy
  másik lépés kommentje, nem ennek a szeletnek a relokációs blokkja —
  jegyzem, nem javítom.
  **A tesztelést és a kódot közvetlenül futtatva, nem csak átolvasva
  ellenőriztem:** friss venv-be telepítve a `.github/requirements-dev.txt`
  + `.github/requirements.txt` függőségeit, `pytest
  tests/test_reconcile_step.py -q` → **56 passed** (a fájlban ténylegesen
  56 `def test_` van, egyezik); a teljes `pytest` suite → **823 passed**,
  0 failed, kilépési kód 0 (823−56=767, egyezik a jelentett „up from
  767" állítással, ami maga is egyezik a szelet-3 zárónapló saját
  mérésével); `ruff check .` a teljes repón, majd külön `ruff check
  scripts/state/reconcile_step.py tests/test_reconcile_step.py` → mindkettő
  „All checks passed!"; a módosított `ci_dev_workflow.yml`
  `yaml.safe_load`-dal betöltve hiba nélkül fut. **`actionlint` v1.7.12 +
  `shellcheck` v0.10.0 saját letöltésű binárisokkal (a jelentett
  verziószámokkal `--version`/`-version`-nel pontosan egyezően)
  futtatva mind a négy workflow-fájl ellen** (`ci_dev_workflow.yml`,
  `ci_prod_workflow.yml`, `ci_prod_audit.yml`, `ci_code_checks.yml`) →
  nulla találat, kilépési kód 0.
  **Miért csak a 4. szelet és nem lezárás:** az utolsó, ötödik blokk
  (`prepare_validate_convert` „Build pipeline bundle" lépése, 111 sor) és
  a kisebb blokkok hosszú farka érintetlen. A nyitott halmaz **nem**
  mozdul — a 4.1 továbbra is részleges, a lezárt tételszám marad
  **37/52**. **Következő valós, nem-docs tétel:** `prepare_validate_convert`
...
- **2026-08-24 — 4.4 „(a)" szelet leszállítva és Kwame által
  önállóan verifikálva: `docs/index.html` + README STATS-blokk kikerült
  a `dev`-re commitolt fájlok közül, a Pages-publikálás mégis változatlanul
  elérhető.** Ez a fenti bejegyzésben azonosított, még nyitott „(b) opció"
  (generált artefaktumok teljes kivezetése a branch-ről) egy scope-olt
  első szelete: kizárólag `docs/index.html` és README STATS-blokkja,
  `outputs/results/`/`outputs/reports/` (`stats.json`-nal együtt)
  szándékosan hatókörön kívül és — verifikálva — ténylegesen érintetlen.
  Commit `da85f9c` (`refactor(ci): stop committing docs/index.html +
  README stats to dev (4.4)`), öt fájl: `ci_code_checks.yml`,
  `ci_dev_workflow.yml`, `README.md`, `scripts/state/merge_verification_
  results.py`, `tests/test_merge_verification_results.py` — `git show
  --stat da85f9c` szerint pontosan ezek, semmi más, a hatókör nem
  csúszott. Egy második, kapcsolódó commit (`644669f`, „docs(skills): fix
  stale pipeline-ci-gotchas after item 4.4's Pages change") ugyanazon a
  napon javította a `pipeline-ci-gotchas` skill H szakaszának első
  bejegyzését.
  **A mag-állítás (Pages-publikálás git-commit nélkül) végigkövetve mindkét
  útvonalon, a tényleges YAML-ból, nem a commit-üzenetből elfogadva:**
  (a) `ci_code_checks.yml` — `regenerate_console` „Regenerate and commit if
  changed" lépése után közvetlenül, semmilyen köztes lépés nélkül fut az
  „Upload console artifact for Pages" (`name: console-${{ github.run_id
  }}`, `path: docs/`, 1 napos retenció); `publish_console` gate-je
  `needs.regenerate_console.result == 'success'`-ra egyszerűsödött,
  checkout helyett `download-artifact`-tal tölti le **ugyanazt a
  `console-${{ github.run_id }}` nevet** — egyezés megerősítve. (b)
  `ci_dev_workflow.yml` — `update_dashboard` „Merge verification results,
  generate stats and commit" lépése után szintén közvetlenül, köztes lépés
  nélkül fut ugyanaz az upload-lépés, ugyanazzal a névvel; `deploy_pages`
  gate-je `needs.update_dashboard.result == 'success'`-ra szűkült (a
  korábbi `'failure'` ág kikerült), checkout helyett ugyanazt a nevet
  tölti le `download-artifact`-tal — egyezés itt is megerősítve. Egy
  névelírás itt a teljes mechanizmust csendben törte volna el; nem történt
  ilyen.
  **Alapértelmezett lépés-sorrend „skip, ha az előző lépés hibázott"
  szemantikája megerősítve, nem csak feltételezve:** mindkét, artifact-ot
  feltöltő lépés előtti lépés (`Regenerate and commit if changed`, illetve
  `Merge verification results, generate stats and commit`) sima `shell:
  bash` + `set -euo pipefail`, `continue-on-error` vagy egyedi `if:`
  felülbírálás nélkül — tehát a job alapértelmezett `continue-on-error:
  false` viselkedése érvényes, egy hibázó lépés valóban ledobja a jobot, és
  az utána következő, explicit `if:` nélküli upload-lépés valóban
  kimarad. Egyik upload-lépésnek sincs saját `if:`-je; a kommentek ezt
  tudatos döntésként indokolják, a kód ezt alátámasztja.
  **A szűkített `deploy_pages` gate (`success` only, `failure` ág
  eldobva) indoklása a tényleges lépéssorrendből megerősítve:** az
  `update_dashboard` jobban az „Upload console artifact for Pages" lépés
  ma is közvetlenül a „Merge verification results, generate stats and
  commit" lépés után jön, semmi más köztes lépés nincs a kettő között —
  tehát egy `'failure'` eredmény valóban azt jelenti, hogy a
  commit-releváns lépés hibázott, ami miatt az upload-lépés eleve nem
  futott le, tehát nincs artifact, amit letölteni. Az indoklás helyes.
  **Hatókör-határ ellenőrizve, nem csak állítva:** `merge_verification_
  results.py`-ban `STAGE_PATHS = ["outputs/results/", "outputs/reports/"]`
  — mindkettő benne maradt. `scripts/state/open_promotion_pr.py` — `git
  diff da85f9c~1 da85f9c -- scripts/state/open_promotion_pr.py` **üres**,
  tehát a `stats.json` contents-API-s visszaolvasása `dev`-ről
  érintetlen; a script legutóbbi módosítása a mai, ehhez a tételhez nem
  kapcsolódó `8566f90` (4.1 szelet 3). README STATS-blokkjában mindkét
  fajta tartalom valóban jelen van, ahogy a felhasználó előzetesen
  megállapította: élő shields.io dynamic-JSON badge-ek (`stats.json`-t
  `main`-ről olvasva, ezek továbbra is pontosak maradnak) ÉS két
  bebetonozott prózasor (`27 of 28 rules are currently out of testing
  scope…`, `Last live verification: 2026-08-17 20:08:36 UTC…`), amelyek
  mostantól elavulhatnak, amíg valaki kézzel nem futtatja újra a
  generátort és nem commitolja a README-t.
  **`regenerate_console` saját `if:`-je megerősítve:** `(github.event_name
  == 'push' || github.event_name == 'workflow_dispatch') && github.ref ==
  'refs/heads/dev'` — tehát a `publish_console` egyszerűsített
  `needs.regenerate_console.result == 'success'` gate-je valóban helyesen
  kizár minden PR-t és minden más branch-et, mert a felsőbb job eleve nem
  fut le azokon.
  **Tesztek, ténylegesen lefuttatva (venv, `.github/requirements-dev.txt`
  + `.github/requirements.txt`):** `pytest tests/test_merge_verification_
  results.py -q` → **31 teszt, mind zöld** (29-ről nőtt, a két új
  `STAGE_PATHS`-tesztnek megfelelően). Teljes suite → **863 teszt, mind
  zöld** (861-ről nőtt, a 4.1 szelet-5 utáni számhoz képest +2). `ruff
  check .` → tiszta. `yaml.safe_load` mindkét módosított workflow-fájlon
  → hibátlan. `actionlint` (helyi binárissal, `-shellcheck` integrálva)
  mind a négy workflow-fájlon (`ci_dev_workflow.yml`, `ci_code_checks.yml`,
  `ci_prod_workflow.yml`, `ci_prod_audit.yml`) → **0 találat**. Az új
  `upload-artifact`/`download-artifact` SHA-pinek (`043fb46d…` / v7.0.1,
  `3e5f45b2…` / v8.0.1) megegyeznek minden más, a repóban már meglévő
  `actions/upload-artifact`/`download-artifact` hivatkozással — nem egyedi,
  eltérő pin.
  **A kísérő skill-javítás (`644669f`) tartalmilag pontos, nem csak a
  commit-üzenetet ismétli meg:** a `pipeline-ci-gotchas` H szakaszának
  átírt első bejegyzése helyesen írja le az új gate-et
  (`needs.regenerate_console.result == 'success'`) és az
  artifact-transport mechanizmust — mindkettő egyezik a fenti, közvetlenül
  a YAML-ból végzett verifikációval.
  **Verdikt:** minden állítás megállja a helyét, semmilyen eltérés nem
  került elő. A checkbox `[ ]` marad — ahogy a fenti 4.4 tételszöveg és a
  közvetlenül megelőző Napló-bejegyzés is rögzíti, a maradék „(b)"
  irány (`outputs/results/`/`outputs/reports/` branch-ről való
  kivezetése) továbbra is teljesen nyitott, ez a bejegyzés csak azt
  könyveli, hogy a „(a)" szelet (docs/index.html + README STATS-blokk)
...
- **2026-08-29 — 1.1, 1.2, 2.4 lezárva, Kwame verifikálta. Számláló 38/52 → 41/52.**
  A fenti (ugyanaznapi) drift-bejegyzés a `docs/architecture/repo-terkep.hu.md`-t
  nevezte meg a 1.1/1.2 egyetlen fennmaradó felületeként és a 2.4 teljes
  tárgyaként. Chloe elvégezte a passzt (`f7bec42`): a drift-gate narratíva
  helyére a Sigstore / `gh attestation verify` provenance-kapu került
  (`repo-terkep.hu.md:88,107,111–114`), a `ci_prod_audit.yml` felkerült 4.
  workflow-ként saját szekcióval (`:121`), a dev (11) + prod (4) job-táblák
  újraépítve, a roster 10 specialistára javítva (`:29`), és minden elavult
  sor-/job-/szabályszám frissítve.
  **Verifikálva a valós repo-állapottal szemben, nem Chloe commit-üzenetéből:**
  – **1.1**: `grep` a README-re és mind az öt `docs/architecture/*.md`-re *aktív*
    drift-gate-állításra (a „no longer / replaced / used to / már nincs / helyette"
    kontextusokat kiszűrve) → **0 találat**. Mind az öt hely (README átírva,
    `pipeline_overview.md`, `scripts_reference.md`, `data_flow.md`, `threat_model.md`,
    `repo-terkep.hu.md`) helyesen írja le az attesztációs kaput. A `.github/dependabot.yml`
    fejléc-kommentje (a tétel egyik eredeti horgonya) is helyes: „register item 2.11",
    dev saját reprodukálhatóság, nem a 3.2 előtti prod-újrakonverzió.
  – **1.2**: `repo-terkep.hu.md:121` `ci_prod_audit.yml` szekció megvan; `CLAUDE.md:22`,
    a Jamal-ügynökfájl, `data_flow.md`, `scripts_reference.md`, `threat_model.md`,
    `pipeline_overview.md` mind hivatkozik a 4. workflow-ra; a README-ből eltűnt a
    „three workflows" állítás. Minden felület egyezik.
  – **2.4**: a coordinator független mérése (`ci_dev` 2623 sor / 11 job, `ci_prod`
    563 / 4, `ci_code_checks` 808 / 6, `ci_prod_audit` 356 / 2, 28 szabály, 7
    requirements-pin) egyezik a `repo-terkep.hu.md` új számaival; a `:106`/`:130`
    drift-gate-indoklás lecserélve.
  **1.3 és 2.1 szándékosan nyitva marad (nem flippelve):** a **2.1**
  `scripts_reference.md`-ből még ~10 script hiányzik (`env.py`, `meta_sidecar.py`,
  `rules.py`, `splunk_client.py`, `splunk_ns.py`, `summary.py`, `verdict_history.py`,
  `check_detect_id_uniqueness.py`, `check_spl_syntax.py`, `new_rule.py`); a **1.3**
  mögöttes felületei (README 11-csomópontos gráf, `pipeline_overview.md` job-táblák)
  ténylegesen javultak, de a tétel törzsszövege még „9 job" / „két jobja van" —
  Gaz döntése, hogy egy gyors verifikáció után zárja-e.
  **Nyitott halmaz (11): 1.3, 2.1, 2.2, 2.3, 2.5, 2.6, 2.7, 2.9, 2.13, 2.14
  (mind Chloe/docs, deprioritizált), + 4.4 (b) (Jamal). A következő nem-docs,
  csapat-végrehajtható tétel változatlanul a 4.4 (b).** **Modell:** Sonnet 5.

- **2026-08-29 — 1.3 és 2.1 lezárva, Kwame verifikálta. Számláló 41/52 → 43/52.**
  Chloe passza (`8bc3dc6`).
  **1.3 — verifikálva a valós YAML-lal szemben:** a `.github/workflows/` job-számai
  ténylegesen **11 / 4 / 2 / 6** (`jobs:` alatti kulcsok, nem az `on:`-triggerek).
  `docs/architecture/pipeline_overview.md` dev job-táblája (`:401–411`) mind a 11
  jobot listázza — az `update_dashboard`, `persist_results_fallback`,
  `notify_pipeline_status` felkerült —, a prod-tábla (`:419–422`) 4 sor, a
  `ci_code_checks`-tábla (`:434–439`) 6 sor. A README nem igényelt módosítást: a
  11-csomópontos Mermaid job-gráfja már helyes volt. `grep` „single job" / „two
  jobs" a README-re és a `docs/architecture/`-ra: az egyetlen „Two jobs:" találat
  (`scripts_reference.md:218`) helyesen a `ci_prod_audit.yml` **valódi** 2 jobjára
  vonatkozik, nem elavult. A README és a `pipeline_overview.md` egymással és a
  valós számokkal is egyezik. A tétel törzsszövegében maradt „9 job" / „két jobja
  van" a *2026-08-18-i audit-időpont* állapotát idézi — a horgony-felületek
  (`README.md`, `pipeline_overview.md`) javítva vannak, a tétel literális kérése
  teljesült. Chloe által jelzett, **nem 1.3 hatókörű** maradék (docs-drift-queue):
  a `pipeline_overview.md` szakaszonkénti narratívája + verdikt-életciklus prózája
  helyenként a 2026-08-22 előtti világot írja; a „Custom Claude Code subagents
  involved" szekció régi csupasz slugokat használ és ~5 ügynök hiányzik belőle —
  ez utóbbi a **2.3** területe.
  **2.1 — verifikálva:** mind a 10 korábban hiányzó script most dokumentált a
  `scripts_reference.md`-ben (`new_rule.py`, `check_detect_id_uniqueness.py`,
  `check_spl_syntax.py` — helyesen `scripts/deploy/` alatt jelölve, nem
  `scripts/validate/`, ahogy a register listája tévesen mondta —, mind a 7
  `scripts/lib/` modul). A `rule_version.py` sehol nem szerepel élő scriptként
  (a 2026-08-29-i törlése után helyes). A coordinator ellenőrizte a 18 új
  teszt-fájl létezését és a 3 kapu-script helyét. Chloe által jelzett maradék
  (docs-drift-queue, nem 2.1-blokkoló): a „Supporting configuration" tábla nem
  listázza a `.github/requirements-deploy.txt`-t.
