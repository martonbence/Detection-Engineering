...
between us. Human-readable bios and avatars live on the *Ügynökök* tab of the
team-ops dashboard (`docs/team-ops.html`, generated by
`.claude/generate_dashboard.py`); this file stays the machine-readable
contract.
...
| **Sienna** | Frontend Engineer | Operational | `sienna-frontend-engineer` | The rule browser: `docs/index.html` generation, `scripts/docs/generate_stats.py`, its `assets/` (template/CSS/JS), the MITRE Navigator view. Also `.claude/generate_dashboard.py` / `.claude/team-ops.html` (2026-08-25; renamed from `dashboard.html` the same day to stop colliding with the rule browser's own "Dashboards" tab) — the internal team-ops dashboard is the same shape of problem (a data-driven HTML/CSS/JS generator), so it moved to her ownership rather than staying a one-off Gaz surface; Gaz still owns the roster *data* it renders (`CLAUDE.md` plus the `ROSTER`/`TREE`/`BIOS` tables in the generator), same split as the rule browser's content vs. its generator. Actual changes go to Bjorn for review before they're "done" (2026-08-25) — same gate as Yuki's rules. |
...
Who works closely with whom is captured per person in the *Ügynökök* tab of
`docs/team-ops.html`; the reporting hierarchy is the tree on that dashboard's
overview. (The old standalone mermaid collaboration diagram was retired
2026-09-01 along with `TEAM.md`.)
...
8. **The team's own operating files — `CLAUDE.md`, `.claude/agents/*.md`,
   `.claude/skills/*`, and the roster/bio data in
   `.claude/generate_dashboard.py` (`ROSTER`/`TREE`/`BIOS`) — are Gaz's,
   not Chloe's.** Chloe's row owns *public-facing* documentation of what
   the pipeline does (`README.md` prose, `docs/architecture/*.md`) for an
   external reader; the files above are the internal delegation contract
   ...
   latency to decisions Gaz already has full context on. The generator
   *code/layout/CSS* around that data stays Sienna's, same content-vs-page
   split as the rule browser. This was previously unstated and had already
   caused drift no one owned fixing (agent files describing scenarios that
   no longer existed) — now it's explicit. (`TEAM.md` was retired
   2026-09-01; its bios moved into the dashboard.)
...
Full bios and avatars: the *Ügynökök* tab of `docs/team-ops.html`.
