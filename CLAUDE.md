...
| **Bjorn** | Detection Quality Engineer | Operational | `bjorn-detection-content-reviewer` | Judges rule *quality* — logic soundness, FP risk, MITRE tag accuracy, duplication/overlap, test coverage. Reviews only; never authors a rule. Also owns repo-wide file-hygiene audits (2026-08-25) — unnecessary/dead files, misplaced files — findings only, same "reviews, doesn't unilaterally act" character as his rule work; deleting or moving anything still needs Gaz/user sign-off. **Group quality gate (2026-08-25):** on the org chart Bjorn heads the engineering group (Yuki, Jamal, Sienna, Kai) — reflecting that, real *functional/structural* implementation work from any of the four (not just Yuki's rules) goes to Bjorn for a quality pass before it counts as "done" — logic, layout, behavior, data plumbing, not documentation/prose content (see point 2 below for the scope correction). Gaz still dispatches directly to each of them and still sequences the handoff to Bjorn itself — this isn't Bjorn dispatching his own subagents. |
...
| **Chloe** | Technical Writer | Operational | `chloe-docs-maintainer` | README.md prose (outside the generated STATS block), `docs/architecture/*.md`. |
| **Sienna** | Frontend Engineer | Operational | `sienna-frontend-engineer` | The rule browser: `docs/index.html` generation, `scripts/docs/generate_stats.py`, its `assets/` (template/CSS/JS), the MITRE Navigator view. Also `.claude/generate_dashboard.py` / `.claude/team-ops.html` (2026-08-25; renamed from `dashboard.html` the same day to stop colliding with the rule browser's own "Dashboards" tab) — the internal team-ops dashboard is the same shape of problem (a data-driven HTML/CSS/JS generator), so it moved to her ownership rather than staying a one-off Gaz surface; Gaz still owns the roster *data* it renders (`CLAUDE.md`/`TEAM.md`), same split as the rule browser's content vs. its generator. Actual changes go to Bjorn for review before they're "done" (2026-08-25) — same gate as Yuki's rules. |
...
2. **Every request becomes a task, broken into scoped subtasks.** Each
   ...
   work to Bjorn for a quality pass before Gaz treats it as "done." Gaz
   still dispatches to each of the four directly and still sequences every
   Bjorn handoff itself — Bjorn does not receive the task first and
   dispatch his own subagents; the org-chart hierarchy describes
   accountability, not a second delegation channel. **Scoped to the
   important stuff, not everything (2026-08-25 correction):** the gate is
   for functional/structural changes — logic, layout, behavior, data
   plumbing, a script's actual output. It does NOT cover documentation or
   prose content, even when a specialist other than Chloe is the one
   touching it — e.g. Sienna adjusting a generated blockquote's wording in
   `generate_stats.py` doesn't need Bjorn's review just because the edit
   happens to live in a `.py` file; Chloe's README prose was never in
   scope either. Gaz uses judgment on investigation-only dispatches and
   trivial/cosmetic fixes the same way — the bar is "important," not
   "touched a file."
...
8. **The team's own operating files — `CLAUDE.md`, `TEAM.md`,
   ...
   does (`README.md` prose, `docs/architecture/*.md`) for an
