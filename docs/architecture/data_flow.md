...
`ci_prod_workflow.yml` makes **no** commits back to `main` — `deploy_to_prod` only downloads the gitignored `.meta.json` sidecars (from the attested dev bundle, not by re-converting) for `deploy_spl_to_splunk.py` to read, and deploys. The `.spl` files it deploys are exactly what `dev` already committed and what the promotion PR carried.

It **does** now commit to `dev`, though, not `main`: `update_dashboard` folds that run's `prod_deploy_report.json` into `outputs/reports/deployment_inventory.json`'s `prod` section, regenerates `docs/index.html`/`stats.json`/`docs/team-ops.html`, and commits and pushes the result to `dev` — the same branch the dashboard machinery already lives on — with the same retry-against-`origin/dev` shape `ci_prod_audit.yml`'s `record_inventory` job uses. `deploy_pages` then republishes from that same-run artifact. This closes the gap where the dashboard kept showing prod on its previous rule version until someone ran `ci_prod_audit.yml` by hand.
