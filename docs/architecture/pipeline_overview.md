...
|---|---|
| `ubuntu-latest` | GitHub-hosted. Used for `prepare_validate_convert`, `open_promotion_pr` and `deploy_pages` in the dev workflow, `announce_lab_offline` in the prod one, and every job in `ci_code_checks.yml` — nothing here needs lab network access. |
