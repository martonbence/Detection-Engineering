...
2. **Dependency vulnerabilities**: run `semgrep_scan_supply_chain` if available, and cross-check with `pip-audit`. The dependencies are now pinned in two files (this changed on 2026-08-03 — an older version of this note said the packages were inline `pip install` calls in the workflows, which is no longer true): `.github/requirements.txt` is the pipeline toolchain installed by both the dev and prod workflows, `.github/requirements-dev.txt` is `pytest`/`ruff` for `ci_code_checks.yml`. Audit both with `pip-audit -r`. Two things worth flagging in a report even when nothing is vulnerable — the pins are **not a full lock** (transitive dependencies float and there are no hashes, so `pip-compile`/`--require-hashes` remains an open gap), and the GitHub Actions in the workflows are pinned only to major tags rather than commit SHAs.
